...
<body class="d-flex flex-column h-100">
    ...
    <div class="{{ $containerSize ?? 'container-fluid' }}">
        ...
        <a class="navbar-brand" href="{{ url('/') }}">
            <img src="{{ asset('img/app-logo.png') }}" class="rounded me-2" alt="{{ config('app.name', 'Laravel') }}" width="25" height="25">
            ...
            <ul class="navbar-nav me-auto">
                    ...
                    @can('access-backend')
                        <li class="nav-item">
                            <a href="{{ url('admin') }}" class="nav-link">
                                <x-heroicon-s-arrows-right-left class="icon me-1" />
                                <span class="d-none d-lg-inline">{{ __('layout/application.navigation.user-menu.management-console') }}</span>
                            </a>
                        </li>
                    @endcan
                @endauth
                ...
                <li class="nav-item">
                    <a href="{{ route('search.results') }}" class="nav-link">
                        <x-heroicon-s-magnifying-glass class="icon me-1" />
                ...
                <li class="nav-item">
                    <a href="https://www.forum.vlaamswoordenboek.be" class="nav-link" target="_blank" rel="noopener noreferrer">
                        <x-heroicon-o-chat-bubble-left-right class="icon me-1" />
                        <span class="d-none d-lg-inline">{{ __('layout/application.footer.community-section.forum') }}</span>
                    </a>
                </li>
                ...
                @if (\App\Models\Blog::count('id') > 0)
                    <li class="nav-item">
                        <a href="{{ route('news:index') }}" class="nav-link">
                            <x-heroicon-s-newspaper class="icon me-1" />
                            <span class="d-none d-lg-inline">{{ __('layout/application.navigation.news') }}</span>
                        </a>
                    </li>
                @endif
                ...
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                        <x-tabler-info-square-rounded class="icon me-1" aria-hidden="true" />
                        <span class="d-none d-lg-inline">{{ __('layout/application.footer.links-section.project-information') }}</span>
                    </a>
                    ...
                    <ul class="dropdown-menu border-0 shadow-sm">
                        @if (app(\App\Settings\ProjectInformationSettings::class)->pageActive)
                            <li>
                                <a class="dropdown-item" href="{{ route('project-information') }}">
                                    <x-tabler-info-square-rounded class="icon text-muted me-1" />
                        ...
                        <li>
                            <a href="{{ route('statistics') }}" class="dropdown-item">
                                <x-heroicon-o-presentation-chart-line class="icon text-muted me-1" />
                                <span>{{ __('pages/statistics.page-title') }}</span>
                            </a>
                        </li>
                    </ul>
                </li>
                ...
                <li class="nav-item">
                    <a href="mailto:contact@vlaamswoordenboek.be" class="nav-link">
                        <x-heroicon-s-envelope class="icon me-1" />
                        <span class="d-none d-lg-inline">{{ __('layout/application.footer.links-section.contact') }}</span>
                    </a>
                </li>
            </ul>
                    ...
                    @if (Route::has('filament.admin.auth.login'))
                        <li class="nav-item">
                            <a class="nav-link {{ active('login') }}" href="{{ route('login') }}">
                                <x-tabler-login-2 class="icon me-1" />
                ...
                @else
                    ...
                    <li class="nav-item">
                        <a href="{{ route('profile:inbox') }}" class="nav-link">
                            <x-heroicon-s-envelope class="icon" />
                            @if (auth()->user()->unreadMessagesCount() > 0)
                                <span class="badge bg-danger position-absolute top-0 start-100 translate-middle rounded-pill ps-2">
                    ...
                    <li class="nav-item dropdown">
                        <a href="#" class="nav-link dropdown-toggle" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                            <x-heroicon-s-user-circle class="icon me-1" />
                            <span class="d-none d-md-inline">{{ Auth::user()->name }}</span>
                        </a>

                        <div class="dropdown-menu dropdown-menu-end border-0 bg-white shadow-sm">
                            <a class="dropdown-item" href="{{ route('account:public', auth()->user()) }}">
                                <x-heroicon-o-user-circle class="text-muted icon me-1" />
                                {{ __('layout/application.navigation.user-menu.public-profile') }}
                            </a>
                            <a class="dropdown-item" href="{{ route('profile.settings') }}">
                                <x-heroicon-o-cog-8-tooth class="text-muted icon me-1" />
                                {{ __('layout/application.navigation.user-menu.settings') }}
                            </a>

                            <div class="dropdown-divider"></div>

                            <a class="dropdown-item" href="{{ route('suggestions:index') }}">
                                <x-heroicon-o-queue-list class="text-muted icon me-1" />
                                {{ __('layout/application.navigation.user-menu.my-suggestions') }}
                            </a>

                            <a class="dropdown-item" href="{{ route('bookmarks:index') }}">
                                <x-heroicon-o-book-open class="text-muted icon me-1" />
                                {{ __('layout/application.navigation.user-menu.saved-words') }}
                            </a>

                            <div class="dropdown-divider"></div>

                            <a href="{{ route('logout') }}" class="dropdown-item link-danger" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
                                <x-heroicon-s-power class="icon text-danger me-1" />
                                {{ __('layout/application.navigation.user-menu.lgout') }}
                            </a>
    ...
    <footer aria-label="Hoofd footer" class="footer mt-auto pt-4 pt-md-3 mt-5">
        <div class="container-fluid pt-4 pb-2 py-md-4 text-body-secondary">
            <div class="row">
                <div class="col-lg-3">
