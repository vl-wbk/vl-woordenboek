<?php

return [
    'call-outs' => [
        'information' => [
            'title' => 'Wil je meer weten over dit project?',
            'subtitle' => 'Onze missie en doelen uitgelegd.',
            'text' => 'Met het oog op een duurzame toekomst voor het Vlaams Woordenboek hechten wij veel belang aan correcte en transparante informatie.',
            'actionText' => 'Ja, ik wil meer weten.'
        ],
        'volunteer' => [
            'title' => 'Wil jij onze ploeg versterken?',
            'subtitle' => 'Word vrijwilliger! Jouw inzet is onze kracht.',
            'text' => 'Met een klein en fijn team vrijwilligers verwerken we de suggesties van mensen en en werken we aan de code (open-source) van het Vlaams Woordenboek. We zijn daarom altijd op zoek naar mensen die hier samen met ons aan willen werken.',
            'actionText' => 'Ja, ik heb interesse.'
        ],
        'suggestion' => [
            'title' => 'Ken je een leuk Vlaams woord?!',
            'subtitle' => 'Dat verdient een plaats in ons woordenboek.',
            'text' => 'We willen ons woordenboek zo breed en rijk mogelijk maken. Elk woord dat eigen is aan Vlaanderen, verdient zijn plaatsje. Ken je er een dat nog ontbreekt en dat volgens jou een plekje verdient in het Vlaams Woordenboek? Dien het in als suggestie.',
            'actionText' => 'Dien een suggestie in!'
        ],
        'search' => [
            'title' => 'Snuisteren in het woordenboek?',
            'subtitle' => 'Ontdek een nieuw woord.',
            'text' => 'Niet op zoek naar een specifiek woord? Of gewoon geen inspiratie? En toch goesting om te ontdekken? Kies dan voor een willekeurig woord.',
            'actionText' => 'Begin maar te snuisteren.'
        ]
    ],
    'call-outs.information.subtitle' => 'Onze missie en doelen.',
    'call-outs.information.text' => 'Wij willen het Vlaams Woordenboek toekomstbestendig maken en houden. Daarom willen we hier ook transparant en helder over communiceren.',
    'call-outs.search.text' => 'Niet op zoek naar een specifiek woord? Of gewoon geen inspiratie? En toch goesting om op verkenning te gaan? Kies dan voor een willekeurig woord.',
    'call-outs.suggestion.actionText' => 'Ja, ik heb een suggestie!',
    'call-outs.suggestion.text' => 'We willen ons woordenboek zo breed en rijk mogelijk maken. Elk woord dat eigen is aan Vlaanderen, verdient een plaats. Ken je er een dat nog ontbreekt en dat in het Vlaams Woordenboek thuishoort? Dien het in als suggestie.',
    'call-outs.suggestion.title' => 'Ken je een leuk Vlaams woord?',
    'call-outs.volunteer.actionText' => 'Ja, ik wil graag meedoen!',
    'call-outs.volunteer.subtitle' => 'Kom erbij en word vrijwilliger!',
    'call-outs.volunteer.text' => 'Met een klein en fijn team vrijwilligers verwerken we alle suggesties die binnenkomen en werken we aan de (open-source) code achter het Vlaams Woordenboek. Wil je hier graag aan meewerken? Dan ben je van harte welkom.',
    'jumbotron' => [
        'headings' => [
            'welcome' => 'Welkom op het :applicationName'
        ],
        'leading-paragraph' => 'Het Vlaams Woordenboek is een woordenboek van :articleCount artikelen dat bestaat dankzij een gemeenschap enthousiastelingen. En dankzij al die mensen die ooit een of meerdere bijdragen hebben geleverd.'
    ],
    'jumbotron.leading-paragraph' => 'Het Vlaams Woordenboek bevat momenteel :articleCount artikelen. Die zijn in de loop van de jaren toegevoegd door enthousiaste gebruikers. De site zelf werd en wordt nog steeds onderhouden door een groep ijverige vrijwilligers.',
    'page-title' => 'welkom'
];
